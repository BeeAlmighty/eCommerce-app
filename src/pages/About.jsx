...
export const About = () => {
	...
	return (
		<motion.section
			className='p-[2rem] flex flex-col min-lg:px-[7rem] min-[700px]:px-[3rem] min-xl:px-[10rem] py-[6rem] font-CormorantU gap-[1.5rem]'
			initial={{ opacity: 0.4 }}
			animate={{ opacity: 1 }}
		...
		>
			...
			<p className='text-[1.3rem] min-md:text-[1.5rem]'>
				Hello there! I'm{' '}
				<a
					href='https://moses-maduakonam.vercel.app/'
					target='_blank'
					className='text-purple hover:opacity-90 ease-in duration-100'
				>
					Moses Maduakonam
				</a>
			...
			<div className='flex justify-around w-[50%] mx-auto'>
				<a
					href='https://www.linkedin.com/in/moses-maduakonam'
					target='_blank'
				>
					<FaLinkedin className='text-[2rem] min-md:text-[3.5rem] text-gray-500 duration-150 ease-in hover:text-sky-700 cursor-pointer' />
				</a>
				<a
					href='https://www.github.com/beealmighty'
					target='_blank'
				>
					<FaGithub
						className={`text-[2rem] min-md:text-[3.5rem] text-gray-500 duration-150 ease-in ${
							state.isDark ? 'hover:text-white' : 'hover:text-black'
						} cursor-pointer`}
					/>
				</a>
			</div>
