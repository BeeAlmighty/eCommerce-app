...
export const Product = () => {
  ...
  return (
    <>
      {data && data.images && 
        <section className='min-lg:px-[3rem] min-md:px-[3rem] min-xl:px-[10rem] p-[1rem] grid gap-[2rem] items-center min-lg:grid-cols-2 min-md:grid-cols-2 py-[3rem]'>
          <div className='flex flex-col gap-[1rem] min-xl:flex-row-reverse relative'>
            <img src={data.images[state.selectedImage]} alt={data.title} className='h-[25rem] w-full rounded-lg' />
            <Link to='/products'>
              <button className={`${state.isDark ? 'border-gray-400 ' : 'border-gray-300 text-white'} font-CormorantU border-1 text-[0.9rem] w-fit py-[0.25rem] px-[0.5rem] rounded-4xl cursor-pointer self-end absolute top-[0.5rem] right-[0.5rem] bg-black flex items-center gap-0.5`}>
                <FaAngleLeft />
                Back
              </button>
            </Link>
            <div className='grid grid-cols-3 gap-[1rem] w-[65%] mx-auto min-xl:grid-cols-1 min-lg:gap-[1rem'>
              {data.images && data.images.map((image, index) => (
                <img key={index} src={image} alt={data.title} className={`rounded-lg ${state.selectedImage === index && 'opacity-20 ease-in duration-200 bg-white'} cursor-pointer`} onClick={() => handleProductImage(index)} />
              ))}
            </div>
          </div>
