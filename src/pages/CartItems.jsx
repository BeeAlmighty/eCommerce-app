...
export const CartItems = () => {
  ...
  return (
    <section className='flex flex-col items-center p-[1rem] gap-[2rem] min-lg:px-[7rem] min-md:px-[3rem] min-xl:px-[10rem] py-[4rem] max-w-[60rem] mx-auto'>
      ...
      {state.cartTotalQuantity > 0 ? 
        <div className='flex flex-col gap-[1rem] font-CormorantU'>
          <div className='flex flex-col gap-[1rem] font-CormorantU'>
            {data.map(item => {
            return <Cart data={item} key={item.id} qty={item.quantity} price={`$ ${(item.price * item.quantity).toLocaleString(undefined, { minimumFractionDigits: 2,maximumFractionDigits: 2})}`} />
            })}
          </div>
          <div className='bg-slate-300 p-[1rem] rounded-xl text-black flex flex-col gap-[0.5rem] font-CormorantU'>
            ...
            <div className='flex justify-between text-[1.4rem]'>
              <span>
                Item's total ({state.cartTotalQuantity})
              </span>
              <span className='font-bold'>
                {'$ ' + grossPrice.toLocaleString(undefined, { minimumFractionDigits: 2,maximumFractionDigits: 2})}
              </span>
            </div>
            <div className='flex justify-between font-bold'>
              ...
              <span className='text-gray-700 text-[1.2rem]'>
                {'+ ' + '$ ' + vatPrice.toLocaleString(undefined, { minimumFractionDigits: 2,maximumFractionDigits: 2})}
              </span>
            </div>
            <div className={`flex justify-between items-center font-bold`}>
              ...
              <span className={`${discountedPrice > 0 ? 'text-green-500' : 'text-gray-400'} text-2xl`}>
            ...
            <div className='flex justify-between items-center font-bold'>
              ...
              <span className='text-3xl'>
                {'$ ' + (grossPrice + vatPrice - discountedPrice).toLocaleString(undefined, { minimumFractionDigits: 2,maximumFractionDigits: 2})}
              </span>
            </div>
            ...
            <button className='button bg-purple text-white text-[1.2rem] hover:bg-purple-700 ease-in duration-150 focus:border-2 focus:border-white'>
              Checkout ({`$ ${(grossPrice + vatPrice - discountedPrice).toLocaleString(undefined, { minimumFractionDigits: 2,maximumFractionDigits: 2})}`})
            </button>
          </div>
        </div>
        : 
